...
export function NewsFeed({ compact = false, maxItems }: NewsFeedProps) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setArticles(maxItems ? sorted.slice(0, maxItems) : sorted);
      } catch {
        setArticles([]);
      } finally {
        setLoading(false);
    ...
    return (
      <Card>
        <div className="flex items-center gap-2 mb-4">
          <Newspaper className="h-4 w-4 text-ds-text-accent" />
          <h2 className="text-sm font-semibold text-ds-text-primary">HMRC News</h2>
        </div>
    ...
    <Card className="p-0 overflow-hidden">
      <div className="flex items-center gap-2 px-6 py-4 border-b border-ds-border-default">
        ...
        <h2 className="text-sm font-semibold text-ds-text-primary">
          HMRC News & Tax Updates
        </h2>
      </div>

      <div className={compact ? "divide-y divide-ds-border-default" : "p-4 space-y-3"}>
        {articles.map((article, index) => {
          const config = impactConfig[article.impact] || impactConfig.medium;
            ...
            >
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <h3 className={`font-medium text-ds-text-primary ${compact ? "text-xs" : "text-sm"}`}>
                        {article.title}
                      </h3>
                      <span
              ...
              <AnimatePresence>
                {isExpanded && (
                  ...
                  >
                    <p className="text-xs text-ds-text-secondary leading-relaxed mt-2 pl-0">
                      {article.summary}
                    </p>
                  </motion.div>
                )}
