...
export function ProgressTracker({
  ...
  return (
    <motion.div
      ...
      className="rounded-xl border border-ds-border-default bg-ds-bg-tertiary p-6"
    >
      <div className="flex items-center justify-between mb-5">
        <h3 className="text-sm font-semibold text-ds-text-primary">
          Consultation Progress
        </h3>
        <span className="text-xs font-medium text-ds-text-accent">
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={i} className="flex flex-1 items-start">
              ...
              {/* Connector line */}
              {i < steps.length - 1 && (
                <div className="flex items-center h-10 w-full max-w-[60px] px-1">
                  <div
                    className={cn(
                      "h-0.5 w-full rounded-full transition-colors duration-300",
                      isCompleted ? "bg-ds-feedback-success" : "bg-ds-border-default"
                    )}
                  />
                </div>
              )}
